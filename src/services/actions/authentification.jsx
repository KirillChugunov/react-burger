import { checkUserInfo, getAuthCoockie, requestLogin, requestRegistrationNewUser } from "../Api/api";
...
export const AuthUserOnLoad = () => {
  return function (dispatch) {
    getAuthCoockie()
    .then((res) => (setCookie("refreshToken", res.refreshToken),setCookie("accessToken", res.accessToken),
    dispatch({
    ...
    .then((res) => (checkUserInfo(res.accessToken))
    .then(res => dispatch({
...
export const handleRegistration = (name, email, password) => {
  return function (dispatch) {
    requestRegistrationNewUser(name, email, password)
     .then((res) => (dispatch({
         type: REGISTRATION,
         name:res.user.name,
