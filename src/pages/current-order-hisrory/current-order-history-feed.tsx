...
export const CurrentOrderHistoryFeed:FunctionComponent = () => {
  ...
  const unicIngredients = orderIngredientsArr?.filter(function (x:any, i:any, a:any) {
    return a.indexOf(x) === i;
  }); //// новый массив для рендера из уникальныъ элементов

  const unicIngredientsWithCount = unicIngredients?.map((element:Tingredient) => ({
    ...element,
    count: orderIngredientsArr?.filter((item:Tingredient) => item._id === element._id)
      .length,
